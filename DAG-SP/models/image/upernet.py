...
class UperNetHead(nn.Module):
    ...
    def __init__(self, n_classes, in_channels, channels=512, pool_scales=[1,2,3,6]):
        ...
        psp_output_channels = len(self.pool_scales) * self.channels
        bottleneck_input_channels = self.in_channels[-1] + psp_output_channels
        
        self.bottleneck = UperNetConvModule(
            bottleneck_input_channels,
            self.channels,
            kernel_size=3,
            padding=1,
        )
...
class UperNetForSemanticSegmentation(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        backbone_channels = self.backbone.channel_list
        ...
        if config.backbone_model_weights is not None:
            self.load_backbone_weights(config.backbone_model_weights)
    ...
    def load_backbone_weights(self, weight_path):
        ...
        for key, value in state_dict.items():
            if key.startswith('backbone.') or key.startswith('patch_embed') or key.startswith('blocks'):
    ...
    def forward(
        ...
        if pixel_values.shape[-2:] != logits.shape[-2:]:
        ...
        return SemanticSegmenterOutput(
            loss=loss,
            logits=logits,
            hidden_states=features if output_hidden_states else None,
        )
